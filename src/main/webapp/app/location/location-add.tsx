...
export default function LocationAdd() {
  ...
  useDocumentTitle(t('location.add.headline'));
  ...
  const createLocation = async (data: LocationDTO) => {
    ...
    try {
      ...
      navigate('/locations', {
            state: {
              msgSuccess: t('location.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('location.add.headline')}</h1>
      <div>
        <Link to="/locations" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('location.add.back')}</Link>
      </div>
    </div>
