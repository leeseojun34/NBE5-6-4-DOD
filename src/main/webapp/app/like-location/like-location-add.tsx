...
export default function LikeLocationAdd() {
  const { t } = useTranslation();
  useDocumentTitle(t('likeLocation.add.headline'));
  ...
  const createLikeLocation = async (data: LikeLocationDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.post('/api/likeLocations', data);
      navigate('/likeLocations', {
            state: {
              msgSuccess: t('likeLocation.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('likeLocation.add.headline')}</h1>
      <div>
        <Link to="/likeLocations" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('likeLocation.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createLikeLocation)} noValidate>
      ...
      <input type="submit" value={t('likeLocation.add.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 mt-6" />
    </form>
  </>);
}
