...
export default function GroupMemberAdd() {
  ...
  useDocumentTitle(t('groupMember.add.headline'));
  ...
  const createGroupMember = async (data: GroupMemberDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.post('/api/groupMembers', data);
      navigate('/groupMembers', {
            state: {
              msgSuccess: t('groupMember.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('groupMember.add.headline')}</h1>
      <div>
        <Link to="/groupMembers" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('groupMember.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createGroupMember)} noValidate>
      ...
      <input type="submit" value={t('groupMember.add.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 mt-6" />
    </form>
  </>);
}
