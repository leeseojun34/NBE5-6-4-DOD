...
export default function LocationCandidateAdd() {
  const { t } = useTranslation();
  useDocumentTitle(t('locationCandidate.add.headline'));
  ...
  const createLocationCandidate = async (data: LocationCandidateDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.post('/api/locationCandidates', data);
      navigate('/locationCandidates', {
            state: {
              msgSuccess: t('locationCandidate.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('locationCandidate.add.headline')}</h1>
      <div>
        <Link to="/locationCandidates" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('locationCandidate.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createLocationCandidate)} noValidate>
      ...
      <input type="submit" value={t('locationCandidate.add.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 mt-6" />
    </form>
  </>);
}
